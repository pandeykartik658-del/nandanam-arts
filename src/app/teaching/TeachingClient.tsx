...
export default function TeachingClient({ 
  ...
  sanityDanceImages?: any[]; 
  sanityMusicImages?: any[];
}) {
  const finalDanceImages = sanityDanceImages?.length 
    ? sanityDanceImages.map((img: any) => ({ src: img.image, caption: img.caption || "" }))
    : defaultDanceImages;
    
  const finalMusicImages = sanityMusicImages?.length 
    ? sanityMusicImages.map((img: any) => ({ src: img.image, caption: img.caption || "" }))
    : defaultMusicImages;
  ...
  return (
    <div className="relative noise-overlay min-h-screen">
      ...
      {/* HERO SECTION */}
      <section id="hero" className="min-h-[40vh] flex flex-col items-center justify-center text-center px-6 relative pt-12 pb-0 z-10">
        ...
        >
          ...
          >
            The teaching wing of the foundation Nandanam Center for Arts conducts classes for Carnatic Vocal and Bharatanatyam, training several aspiring students from all age groups.
          </motion.p>
        </motion.div>
      ...
      {/* DANCE WING */}
      <section id="dance-wing" ref={danceRef} className="max-w-[1100px] mx-auto px-6 pt-12 pb-12 relative z-10">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-16 items-center">
          ...
          <motion.div initial={{ opacity: 0, x: 40 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.8, delay: 0.4 }}>
            <DynamicRotatingShowcase images={finalDanceImages} type="dance" />
          </motion.div>
        </div>
      </section>
      ...
      <section id="music-wing" ref={musicRef} className="max-w-[1100px] mx-auto px-6 pt-12 pb-12 relative z-10">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-16 items-center">
          ...
          <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.8, delay: 0.2 }} className="order-1 lg:order-2">
            <div className="grid grid-cols-1 gap-6">
              <div><PillarCard icon={<CustomIcons.Resonance />} title="About" desc="The music wing of Nandanam Center for Arts conducts classes for Carnatic vocals, training several aspiring students from all age groups." delay={0} backContent={<div className="w-[84px] h-[84px] sm:w-[120px] sm:h-[120px] bg-gradient-wine-shift relative opacity-80" style={{ WebkitMaskImage: `url(${logo})`, WebkitMaskSize: "contain", WebkitMaskRepeat: "no-repeat", WebkitMaskPosition: "center", maskImage: `url(${logo})`, maskSize: "contain", maskRepeat: "no-repeat", maskPosition: "center" }} />} /></div>
